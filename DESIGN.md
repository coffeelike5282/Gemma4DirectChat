...
- 텍스트가 생성되는 동안 커서 포인트가 깜빡이는 블링킹 애니메이션 추가.
- 생성 중에는 **Stop (중단)** 버튼이 활성화되어 즉각적인 피드백과 제어권을 사용자에게 부여.

### 대화 제어 (Control UI)

- **New Chat (+)**: 헤더 우측 상단 배치. 클릭 시 즉각적인 상태 초기화 및 시스템 프로포트 재배치.
- **Model Settings**: 온도, 토큰 제한 등 상세 파라미터를 글래스모피즘 오버레이 창으로 제공.
